...
def set_seed(seed):
    ...
    # 固定 NumPy 的随机数生成器
    np.random.seed(seed)
    # 固定 PyTorch CPU 的随机数生成器
    torch.manual_seed(seed)
    # 固定 GPU 上的随机数种子（如果有 GPU）
...
if __name__ == '__main__':
    ...
    parser.add_argument('--λ', type=float, default=0.5)
    parser.add_argument('--jitter_scale_ratio', type=float, default=1.2)
    parser.add_argument('--jitter_ratio', type=float, default=0.9)
    ...
    if config.mode == 'train':
        ...
        #print("ablation-restruct strong to weak 0,restruct weak to strong 0")
        #print("ablation-weak-1  strong permutation-1  sinkhorn-0")
