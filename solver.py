    ...
    def train(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            for i, (input_data, labels) in enumerate(self.train_loader):
                ...
                # strong permutation-1  weak permutation-1
                ...
                # strong permutation-0  weak permutation-1
                # z1, _ = DataTransform(input, self.jitter_scale_ratio, self.max_seg, self.jitter_ratio)
                # z1 = z1.transpose(1, 2).to(self.device)
                # z2 = input_data.to(self.device)

                # strong permutation-1  weak permutation-0
                # _, z2 = DataTransform(input, self.jitter_scale_ratio, self.max_seg, self.jitter_ratio)
                # z2 = z2.transpose(1, 2).to(self.device)
                # z1 = input_data.to(self.device)
...
#KLscore = my_kl_loss(attMat1.detach(), attMat2.detach()) + my_kl_loss(attMat2.detach(), attMat1.detach())
    def test(self):
        ...
        for i, (input_data, labels) in enumerate(self.train_loader):
            ...
            #########可视化
            # np.save("p{i}.npy", P.detach().cpu().numpy())
        ...
        ####
        combined_energy = np.concatenate([train_energy, test_energy], axis=0)  #train + test
        thresh = np.percentile(combined_energy, 100 - self.anormly_ratio)             #train + test

        # thresh = np.percentile(test_energy, 100 - self.anormly_ratio)    #test
        print("Threshold :", thresh)
        ####
